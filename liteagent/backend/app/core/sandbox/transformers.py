...
Transforms user code into safely executable scripts with:
- Input injection
- Result extraction
- Error handling
"""
...
from app.core.sandbox.executor import CodeLanguage
...
class CodeTransformer(ABC):
    ...
    def transform(
        ...
        Returns:
            Tuple of (transformed_code, transformed_preload)
        """
        pass
...
try:
    ...
    compiled = env.from_string(_template)
    _result = compiled.render(**inputs)
except Exception as e:
    _error = str(e)
...
if _error:
    ...
    print(f"<<RESULT>>{{json.dumps(_result)}}<<RESULT>>")
'''
    ...
    def transform(
        ...
        escaped_template = code.replace('\\', '\\\\').replace('"', '\\"')
